...
pub fn falg_generate_with_qords_bufwriter<T: std::io::Write>(out_writer: &mut BufWriter<T>, qord1: &Vec<Vec<usize>>, qord2: &Vec<Vec<usize>>) -> bool {
    let n = qord1.len();
    let mut falg = falg_init(n);
    let mut b_filled_arr = Vec::<Vec<bool>>::new();
    // init b_filled_arr
    ...
    if missing_cnt == 0 {
        ...
        key_arr.sort();
        // eprintln!("map keys: {key_arr:?}");
        falg_fill_missing_v3_bufwriter(out_writer, 0, &key_arr, &cands_map, &pairs_map, &pairs_inv_map, qord1, qord2, &mut falg, &mut b_filled_arr);
    }
    true
}
...
pub fn falg_fill_missing_v3_bufwriter<T: std::io::Write>(out_writer: &mut BufWriter<T>, cur_idx:usize, key_arr: &Vec<usize>, cands_map: &HashMap<usize, HashSet<usize>>, pairs_map: &HashMap<usize, (usize, usize)>, pairs_inv_map: &HashMap<(usize, usize), usize>, qord1: &Vec<Vec<usize>>, qord2: &Vec<Vec<usize>>, part_falg: &mut Vec<Vec<usize>>, part_b_filled_arr: &mut Vec<Vec<bool>>) {
    if cur_idx == cands_map.len() {
        // test conditions for falg
        if falg_all_tests_no_print(part_falg) {
            writeln!(out_writer, "{:?}", part_falg).unwrap();
        ...
        if !part_b_filled_arr[cur_x][cur_y] {
            for &c in cur_cands {
                ...
                falg_fill_missing_v3_bufwriter(out_writer, cur_idx + 1, key_arr, cands_map, pairs_map, pairs_inv_map, qord1, qord2, part_falg, part_b_filled_arr);
                part_falg[cur_x][cur_y] = 0;
                part_b_filled_arr[cur_x][cur_y] = false;
                for (k,v) in to_fill_map {
                    if part_b_filled_arr[k.0][k.1] {
